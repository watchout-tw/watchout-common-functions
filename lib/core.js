...
export function confirmEmailVerificationWithPayment(id, token) {
  return customGet(
    false,
    `/citizen/emails/${id}/confirm_verification_p/${token}`
  )
}
...
export function createBanner(params) {
  return coreInstance.post('/console/comp/banners', { image: params.image })
}
...
export function getTimelineEvent(timelineID, eventID) {
  return coreInstance.get(`/comp/timelines/${timelineID}/event/${eventID}`)
}
...
export function patchTimelineEvent(timelineID, eventID, params) {
  return coreInstance.patch(`/comp/timelines/${timelineID}/events/${eventID}`, params)
}
