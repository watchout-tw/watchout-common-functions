...
export function getOrderByID(tradeNo) {
  return coreInstance.get(`/store/orders/${tradeNo}`)
}

export function patchOrder(params) {
  return coreInstance.patch(`/store/orders/${params.merchant_trade_no}`, params)
}

export function getOrdersTransactions(tradeNo) {
  return coreInstance.get(`/store/orders/${tradeNo}/transactions`)
}
