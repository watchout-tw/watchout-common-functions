...
function parseContentToList(keyword, content) {
  ...
  return content.replace(regExp, '').split('\n').map(markdown => {
    ...
    if(referenceObj !== false) {
      ...
      links = links ? links.map(markdownLink => {
...
export function spacingOptimizer(text) {
  let punctGroups = [
    {
      label: 'thin',
      list: [PUNCT.COMMA, PUNCT.STOP, PUNCT.PAUSE, PUNCT.COLON, PUNCT.SEMICOL, PUNCT.EXCLA]
    },
    {
      label: 'thick',
      list: [PUNCT.Q]
    ...
    {
      label: 'left',
      list: [PUNCT.L.PAR, PUNCT.L.DBRAC, PUNCT.L.SBRAC, PUNCT.L.QUOTE, PUNCT.L.SQBRAC]
    },
    {
      label: 'right',
      list: [PUNCT.R.PAR, PUNCT.R.DBRAC, PUNCT.R.SBRAC, PUNCT.R.QUOTE, PUNCT.R.SQBRAC]
    }
  ]
  let html = ''
