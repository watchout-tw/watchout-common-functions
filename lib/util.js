...
function zeroPadding(number, digits = 2) {
  return number >= 10 ** digits ? '' + number : ('0'.repeat(digits) + number).slice(-1 * digits)
}
...
export const formatter = {
  ...
  datetime(d) {
    ...
    return dateObj ? dateObj.toLocaleDateString('zh-Hans-TW') + ' ' + [zeroPadding(dateObj.getHours()), zeroPadding(dateObj.getMinutes()), zeroPadding(dateObj.getSeconds())].join(':') : 'NaDT'
  },
  datetimeForFile(d) {
    let dateObj = null
    if(isMysqlTS(d)) {
    ...
    return dateObj ? dateObj.toLocaleDateString('zh-Hans-TW').replace(/\//g, '') + '_' + [zeroPadding(dateObj.getHours()), zeroPadding(dateObj.getMinutes()), zeroPadding(dateObj.getSeconds())].join('') : 'NaDT'
  },
  dateObj(d) {
    let dateObj = null
    if(isMysqlTS(d)) {
