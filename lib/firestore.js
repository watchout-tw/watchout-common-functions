...
async function publishDoc(doc) {
  let date = new Date()
  let docType = doc.referenceObj.type
  let docFSRef = null
  ...
  if(snapshot && snapshot.size === 0) {
    // add doc
    docFSRef = await firestore.collection('docs').add(Object.assign(doc, getPublishInfo(date)))
  } else if(snapshot && snapshot.size > 0) {
    // update doc
    snapshot.forEach(docInSnapshot => {
      if(!docFSRef) {
        docFSRef = docInSnapshot.ref
      }
    })
    if(docFSRef) {
      docFSRef.update(Object.assign(doc, getUpdateInfo(date)))
    }
  }
  if(docFSRef) {
    docFSRef.collection('versions').add({
      date,
      content: docContent
    })
  }
  ...
  let result = null
  if(docFSRef) {
    let docSnapshot = await docFSRef.get()
    result = Object.assign(docSnapshot.data(), { id: docSnapshot.id })

    let contentSnapshot = await docFSRef.collection('versions').orderBy('date', 'desc').limit(1).get()
    if(contentSnapshot) {
      contentSnapshot.forEach(docInSnapshot => {
