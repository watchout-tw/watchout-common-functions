...
// auth
firebase.auth().signInWithEmailAndPassword(firebaseConfig.devUser.email, firebaseConfig.devUser.password).catch(error => {
...
async function getDocs({ pubDest = '*', activeOnly = true, desc = true, tagID = null, lastDocID = null, limit = -1 } = {}) {
  console.log('getDocs', pubDest, activeOnly, desc, tagID, lastDocID, limit)
  let fsRef = firestore.collection('docs')
  if(pubDest !== '*') {
    fsRef = fsRef.where('publishedTo', '==', pubDest)
  }
  ...
  if(lastDocID) {
    let lastDocSnapshot = await firestore.collection('docs').doc(lastDocID).get()
...
async function getDocVersions(docID, orderBy = 'date', order = 'desc') {
  let docRef = firestore.collection('docs').doc(docID)
  let versions = []
...
async function copyDoc(fromID, toID, mutate = false) {
  ...
  let fromRef = firestore.collection('docs').doc(fromID)
  let toRef = firestore.collection('docs').doc(toID)
...
async function calculateUpdateDate(mutate = false) { // eslint-disable-line camelcase
  ...
  let docs = []
  let snapshot = await firestore.collection('docs').orderBy('publishedAt', 'desc').get()
  if(snapshot) {
    snapshot.forEach(doc => {
      docs.push(Object.assign(doc.data(), { id: doc.id }))
    })
  ...
  for(let i = 0; i < docs.length; i++) {
    ...
    docUpdate.metadataUpdatedAt = fsTSToDateObj(doc.publishedAt)
    docUpdate.contentUpdatedAt = fsTSToDateObj(versions[lastVersionIndex].date)
...
async function getNewsletters() {
  let snapshot = await firestore.collection('newsletters').orderBy('publishedAt', 'desc').get()
  let newsletters = []
  if(snapshot) {
...
async function getNewsletter(id) {
  let snapshot = await firestore.collection('newsletters').doc(id).get()
  let newsletter = null
  if(snapshot && snapshot.id && snapshot.data()) {
...
async function addNewsletter(newsletter, customID = null) {
  ...
  let ref = customID ? firestore.collection('newsletters').doc(customID) : firestore.collection('newsletters').doc()
...
async function updateNewsletter(newsletterID, newsletter) {
  ...
  await firestore.collection('newsletters').doc(newsletterID).update(newsletter)
