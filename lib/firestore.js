...
import { getPubDestFromContent } from 'watchout-common-functions/lib/bunko'
...
function makeDoc(type, rawDoc, cachedAuthors, status = 'active') {
  let authors = rawDoc.authors.map(rawAuthor => {
    let personaID = null
    let rawID = rawAuthor.id
    let author = cachedAuthors.find(author => author.content.id === rawID)
    if(author) {
      personaID = author.persona
    }
    return personaID
  }).filter(authorID => authorID !== null)
  let doc = { type, authors }
  if(type === 'ghost') {
    doc.content = rawDoc
    doc.status = status
    ...
    doc.publishedTo = getPubDestFromContent(type, rawDoc).value
  }
  return doc
}
...
async function publishDoc(doc) {
  ...
  doc.content = { id: docContent.id }

  // check if doc is already published
  let snapshot = null
  if(doc.type === 'ghost') {
    snapshot = await firestore.collection('docs').where('type', '==', 'ghost').where('content.id', '==', docContent.id).get()
  }
  // add or update doc
...
async function addAuthor(author) {
  await firestore.collection('authors').add(Object.assign(author, getPublishInfo()))
}
...
async function addVideo(video) {
  await firestore.collection('videos').add(Object.assign(video, getPublishInfo()))
}
...
async function getCollection(id, withItems = true) {
  ...
  if(collection && withItems) {
    let itemsSnapshot = await firestore.collection('collections').doc(id).collection('items').get()
    let items = []
    if(itemsSnapshot) {
