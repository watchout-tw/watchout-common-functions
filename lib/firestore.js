...
import { fsTSToDateObj } from 'watchout-common-functions/lib/util'
...
function cleanUpDoc(doc) {
  if(!doc.imageLicense) {
    doc.imageLicense = null
  }
  authorTypes.forEach(type => {
    if(!(Array.isArray(doc[type.valuePlural]) && doc[type.valuePlural].length > 0)) {
      doc[type.valuePlural] = firebase.firestore.FieldValue.delete()
    }
  })
  if(!(Array.isArray(doc.tags) && doc.tags.length > 0)) {
    doc.tags = firebase.firestore.FieldValue.delete()
  }
  return doc
}
...
async function getProjects({ pubDest = '*', activeOnly = true, limit = -1 } = {}) {
  let fsRef = firestore.collection('projects').orderBy('publishedAt', 'desc')
  if(pubDest !== '*') {
    fsRef = fsRef.where('publishedTo', '==', pubDest)
  }
  if(activeOnly) {
