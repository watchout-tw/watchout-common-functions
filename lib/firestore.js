...
let bunkoAuth = bunkoApp.auth()
let bunkoFS = bunkoApp.firestore()
bunkoAuth.setPersistence(firebase.auth.Auth.Persistence.NONE) // https://firebase.google.com/docs/auth/web/auth-state-persistence
bunkoAuth.onAuthStateChanged(user => {
  console.info('[bunko] authStateChanged', user ? user.uid + '/' + user.email : null)
})

// set default auth to anyone
bunkoAuth.signInWithEmailAndPassword(firebaseConfig.sysUsers.anyone.email, firebaseConfig.sysUsers.anyone.password).catch(error => {
...
async function login({ id = null, email = null, password = null } = {}) {
  // IF id AND no email
  if(id && !email) {
    // login with dev AND get email
    bunkoAuth.signInWithEmailAndPassword(firebaseConfig.sysUsers.devUser.email, firebaseConfig.sysUsers.devUser.password).catch(error => {
      console.error(error.code, error.message)
    })
    let citizenObj = await getCitizen(id)
    if(citizenObj) {
  ...
  // login with email
  if(email) {
    bunkoAuth.signInWithEmailAndPassword(email, password).catch(error => {
      console.error(error.code, error.message)
    })
  }
}
