...
import { getPubDestFromContent, parseReference, makeReference } from 'watchout-common-functions/lib/bunko'
...
async function getDocs(pubDest = null) {
  let fsRef = firestore.collection('docs').orderBy('publishedAt', 'asc')
  if(pubDest) {
...
async function getTags() {
  let snapshot = await firestore.collection('tags').get()
  let tags = []
  if(snapshot) {
...
async function addTag(tag) {
  await firestore.collection('tags').add(Object.assign(tag, getPublishInfo()))
}
