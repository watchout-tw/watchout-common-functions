...
async function initWithAuth(store) {
  try {
    bunkoApp = firebase.app('[DEFAULT]')
  } catch(error) {
    bunkoApp = firebase.initializeApp(firebaseConfig.projects.bunko)
  ...
  // FIXME: add this for now, haven't figured out when to use park.login()
  ...
  bunkoAuth.onAuthStateChanged(user => {
    ...
    console.info('[bunko] auth state changed', user ? user.uid : 'no user')
  })
  console.info('[bunko] firebase app initialized')
}
...
function getFsTS(dateString) {
  let { seconds, nanoseconds } = util.dateObjToFsTS(new Date(dateString))
...
async function countDocs({
  ...
  if(authorID) {
    fsRef = fsRef.where('authors', 'array-contains', authorID)
  }
  if(tagID) {
    fsRef = fsRef.where('tags', 'array-contains', tagID)
  }
  if(year > 0) {
    let startDate = getFsTS(`${year}/1/1`)
    let endDate = getFsTS(`${year + 1}/1/1`)
    fsRef = fsRef.where('publishedAt', '>=', startDate).where('publishedAt', '<', endDate)
  }
  fsRef = fsRef.orderBy('publishedAt', 'desc')
...
async function getDocs({
  ...
  if(top) {
    fsRef = fsRef.where('top', '>=', 0)
    fsRef = fsRef.orderBy('top', 'asc')
  } else {
...
async function getAuthors() {
  const snapshot = await bunkoFS.collection('authors').get()
  let authors = []
  if(snapshot) {
  ...
  // get authors' personas
  let fetchedPersonas = await core.getAuthors()
  for(let i = 0; i < authors.length; i++) {
    let authorID = parseInt(authors[i].persona)
    let personaObj = fetchedPersonas.data.rows.find(persona => persona.id === authorID)
    // check if personaObj is {}
    authors[i].personaObj = personaObj && Object.keys(personaObj).length > 0 ? personaObj : null
  }
...
async function getAuthor(id) {
  ...
  // get persona
  let persona = await core.getPersonaById(author.persona)
...
async function addPromotionEmails(email, from) {
  ...
  promotionEmail = Object.assign(promotionEmail, getPublishInfo())
  let ref = bunkoFS.collection('promotion-emails').doc()
  await ref.set(promotionEmail)
}

async function getCivilguideHash(id) {
  let snapshot = await bunkoFS.collection('civilguide-active-hash').doc(id).get()
  let hashObj = null
  if(snapshot && snapshot.id && snapshot.data()) {
    hashObj = Object.assign(snapshot.data(), { id: snapshot.id })
  } else {
    console.error('Cannot get civilguide hash', id)
  }
  return hashObj
}
