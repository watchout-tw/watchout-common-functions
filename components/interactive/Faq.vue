...
<div class="section faq dark" id="faq">
  ...
  <div class="items tcl-container">
    <div class="item tcl-panel with-top-bottom-margin" v-for="(item, itemIndex) of faq" :key="itemIndex">
      <h2 class="title" v-html="spacingOptimizer(item.question)"></h2>
      <div class="description paragraphs a-text-parent" v-html="markdown(item.answer)" v-show="faqIX[itemIndex].show"></div>
      <a class="toggle a-block font-size-small" v-if="!faqIX[itemIndex].show" @click="faqIX[itemIndex].show = !faqIX[itemIndex].show"><span class="a-target">{{ showDetail }}</span></a>
    </div>
